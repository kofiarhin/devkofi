...
describe("public blog API", () => {
  ...
  it("lists posts returned by IdeaHub API", async () => {
    global.fetch.mockResolvedValue(upstreamResponse({ posts: [publishedPost] }));
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      "http://ideahub-api.test/api/v1/devkofi/blog-posts",
  ...
  it("returns a post by slug", async () => {
    global.fetch.mockResolvedValue(upstreamResponse({ post: publishedPost }));
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      `http://ideahub-api.test/api/v1/devkofi/blog-posts/${publishedPost.slug}`,
      expect.any(Object),
    );
  });
